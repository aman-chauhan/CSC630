...
def ae_cv_train(ae_config, epochs, ids, mode, batch_size):
    train, validation = train_test_split(ids, test_size=0.1, random_state=10, shuffle=False, stratify=None)
    cnt = 0
    key = '{}_{}_{}_{}_{}'.format('resnet', ae_config['resnet_layers'],
                                  ae_config['top_filters'], ae_config['reduction_layers'],
                                  ae_config['neck_layers'])
    ...
    validation_generator = AEDataGenerator(validation, os.path.join('train_ae_data', mode), 'IMG{:06d}.png',
                                           batch_size, 3, True, True)

    # generators for perplane dataset (uncomment the below section, comment the above section)
    # training_generator = AEDataGenerator(train, os.path.join('ae_data', mode), 'IMG{:06d}.png',
    #                                      batch_size, 3, True, True)
    # validation_generator = AEDataGenerator(validation, os.path.join('ae_data', mode), 'IMG{:06d}.png',
    #                                        batch_size, 3, True, True)
...
def main():
    ...
    if os.path.exists('scores.json'):
        d = json.load(open('scores.json'))
    for i in range(1, 4):
        for j in range(64, 129, 32):
            for k in range(4, 8):
                for l in range(1, 5):
                    if '{}_{}_{}_{}_{}'.format('resnet', i, j, k, l) not in d:
                        print('Train: {}={}, {}={}, {}={}, {}={} Started'.format('resnet_layers', i,
                                                                                 'top_filters', j,
                                                                                 'reduction_layers', k,
                                                                                 'neck_layers', l))
                        ...
                        ae_cv_train(ae_config, 50, X_train, 'T2W', 30)
                    print('Train: {}={}, {}={}, {}={}, {}={} Done\n\n'.format('resnet_layers', i,
                                                                              'top_filters', j,
                                                                              'reduction_layers', k,
                                                                              'neck_layers', l))
